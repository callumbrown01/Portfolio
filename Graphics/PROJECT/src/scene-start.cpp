...
void init(void) {
    ...
    // TASK F (for nav):
    // Shaders for Task A-F
    // shaderProgram = InitShader("res/shaders/vStart.glsl", "res/shaders/fStart.glsl");
    // Shaders for Task G-J
    shaderProgram = InitShader("res/shaders/vStartG.glsl", "res/shaders/fStartG.glsl");
    ...
    // bumped this from 0.2 to 0.5 more cosmetic
    ...
    // TASK J: Spotlight
    addObject(55); // Sphere for third light
    // initial position is below the canvas where the light has no effect
    sceneObjs[3].loc = vec4(0.0, -0.1, 0.0, 1.0);
    sceneObjs[3].scale = 0.1;
    sceneObjs[3].texId = 0;
    sceneObjs[3].brightness = 1;
...
void drawMesh(SceneObject sceneObj) {
    ...
    // Set the model matrix - this should combine translation, rotation and scaling based on contents
...
//TASK c adjusting callback methods
//TASK c adjusting ambient and diffuse light
static void adjustAmbientDiffuse(vec2 ad){
    sceneObjs[toolObj].ambient += ad[0];
    sceneObjs[toolObj].diffuse += ad[1];
}
//pc adjusting specular and shine
static void adjustSpecularShine(vec2 sS){
    sceneObjs[toolObj].specular += sS[0];
    //increasing the shininess should 
    //decrease the size of the highlight, occuranace not yet known
    // multiplies the addition factor by 20
    sceneObjs[toolObj].shine += sS[1] * 20;
}
...
// TASK J.2
// The last object from the scene object array is removed and the number of objects is updated
//
...
static void materialMenu(int id) {
    ...
    if(id == 20) { // if the menu item is called the AmbientDiffuse and Specular shine can be adjusted.
        toolObj = currObject;
...
static void mainmenu(int id) {
    ...
    if (id == 55 && currObject >= 0) {
        setToolCallbacks(adjustAngleYX, mat2(400, 0, 0, -400),
			...
			duplicate_Object(currObject);  // calling the Object duplication fn if event occurs

    }
    else if (id == 91 && currObject >= 0) {
		Delete_Object(currObject);          // calling the Object deletion fn if event occurs
        }
    if (id == 99) exit(0);
}
...
static void makeMenu() {
    ...
    glutAddMenuEntry("Move Spotlight", 82);
    glutAddMenuEntry("R/G/B/All Spotlight", 83);
    glutAddMenuEntry("Rotate Spotlight", 84);
...
void reshape(int width, int height) 
{
    ...
    //         the window.
     ...
     //part D and 
    // decrease the side of near dist
    ...
    // if statement to deal with situion if width is greter than height
    if(width >= height){
        left = -nearDist;
		right = nearDist;
    ...
    }else{   // reversing to keep a square{width  less than height}
